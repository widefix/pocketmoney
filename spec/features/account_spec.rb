...
RSpec.feature 'Account', type: :feature do
  ...
  scenario 'User views the account page' do
    ...
    expect(page).to have_link('Account shares', href: account_shares_path(account))
    expect(page).to have_link('Archive', href: archive_account_path(account))
    expect(page).to have_content('Account details')
    expect(page).to have_content(account.name)
    expect(page).to have_content(account.balance)
    expect(page).to have_link(nil, href: new_account_topup_path(account))
    expect(page).to have_link(nil, href: new_account_spend_path(account))
    ...
    expect(transactions_block).to have_link('Cancel', href: transaction_path(transaction))
